        ...
        <div class="p-5 md:p-8 max-w-[1440px] mx-auto w-full">
            ...
            <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <div
                        class="flex items-center gap-2 px-4 py-2 bg-white border border-[#c5c6ce] rounded-lg text-sm text-[#505f76] shadow-sm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
                ...
                {{-- Total Schools --}}
                <div
                    class="premium-card p-6 rounded-xl border border-[#f1f5f9] shadow-sm flex flex-col items-center text-center gap-4 min-h-[140px] justify-center">
                ...
                <div class="premium-card p-6 rounded-xl flex flex-col bg-white">
                    ...
                    <div class="relative flex-1 flex items-center justify-center min-h-[280px]">
                        ...
                        <svg class="w-48 h-48 transform -rotate-90" viewBox="0 0 192 192">
                            ...
                            {{-- On Going (indigo) --}}
                            <circle cx="96" cy="96" r="80" fill="transparent" stroke="#4f46e5" stroke-width="20"
                                stroke-dasharray="{{ number_format($circ, 2) }}"
                                stroke-dashoffset="{{ number_format($circ - $seg1, 2) }}" class="donut-ring" />
                            {{-- Registered (amber) --}}
                            <circle cx="96" cy="96" r="80" fill="transparent" stroke="#d97706" stroke-width="20"
                                stroke-dasharray="{{ number_format($seg2, 2) }} {{ number_format($circ - $seg2, 2) }}"
                                stroke-dashoffset="{{ number_format($circ - $seg1, 2) }}" class="donut-ring" />
                            {{-- Trial (emerald) --}}
                            <circle cx="96" cy="96" r="80" fill="transparent" stroke="#10b981" stroke-width="20"
                                stroke-dasharray="{{ number_format($status_counts['under_review'] / $total * $circ, 2) }} {{ number_format($circ, 2) }}"
                                stroke-dashoffset="{{ number_format($circ - $seg1 - $seg2, 2) }}" class="donut-ring" />
                            {{-- Maintenance (red) --}}
                            <circle cx="96" cy="96" r="80" fill="transparent" stroke="#ba1a1a" stroke-width="20"
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <span
                                class="text-3xl font-bold text-[#111c2d]">{{ number_format($status_counts['total']) }}</span>
                    ...
                    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-3">
                        ...
                        <form method="GET" action="{{ route('admin.dashboard') }}" class="flex gap-2">
                            <select name="state_id" onchange="this.form.submit()"
                                class="text-xs border border-slate-200 rounded-lg p-1.5 focus:border-[#271e6d] bg-white outline-none">
                                <option value="">All States</option>
                                @foreach($states as $s)
                                    <option value="{{ $s->id }}" {{ $selectedState == $s->id ? 'selected' : '' }}>{{ $s->name }}
                                    </option>
                                @endforeach
                            </select>
                            <select name="zone_id" onchange="this.form.submit()"
                                ...
                                @foreach($zones as $z)
                                    <option value="{{ $z->id }}" {{ $selectedZone == $z->id ? 'selected' : '' }}>{{ $z->name }}
                                    </option>
                                @endforeach
                            </select>
                        </form>
                            ...
                            <div class="space-y-1">
                                ...
                                <div class="w-full bg-[#f0f3ff] h-2 rounded-full overflow-hidden">
                                    <div class="h-full rounded-full transition-all duration-700"
                                        style="width: {{ $pct }}%; background: {{ $barColors[$idx % count($barColors)] }};">
                                    </div>
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left dash-table">
                        ...
                        <tbody class="divide-y divide-[#e7eeff]">
                                    ...
                                    <td>
                                        <p class="text-sm font-bold text-[#111c2d]">
                                            <a href="{{ route('admin.schools.show', $s->id) }}"
                                                class="hover:text-[#4f46e5] transition-colors">{{ $s->name }}</a>
                                        </p>
                                                ...
                                                <div class="py-1">
                                                    <a href="{{ route('admin.schools.show', $s->id) }}"
                                                        ...
                                                        <span class="material-symbols-outlined"
                                                            style="font-size:16px;">visibility</span> View Record
                                                    </a>
                                                    <a href="{{ route('admin.schools.edit', $s->id) }}"
                                                        ...
                                                        <span class="material-symbols-outlined"
                                                            style="font-size:16px;">edit</span> Edit Profile
                                                    </a>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="py-12 text-center text-[#505f76] text-sm font-normal">No recent
                                        school records found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left dash-table">
                        ...
                        <tbody class="divide-y divide-[#e7eeff]">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="py-12 text-center text-[#505f76] text-sm font-normal">No audit
                                        activity logged.</td>
                                </tr>
                            @endforelse
                        </tbody>
